...
- `images` (Map of String) Map of image IDs to full OCI references for resolving cg.json. When provided, the chart's values.yaml will be updated with the resolved image references. Each reference must resolve in its registry before the chart is pushed; set HELM_SKIP_IMAGE_VERIFY=true to skip that check.
...
- `package_version` (String) The version of the package to fetch from the package repository, pinned via APK's `name=version` constraint syntax. If not specified, the latest available version will be used.
